...
- En CI Windows, `pnpm tauri build --no-bundle` précompile avec l'overlay de signature avant la connexion Azure ; `azure/login`, le préchargement du jeton et `Build Tauri` restent consécutifs. Cette dernière étape signe le binaire précompilé puis utilise `pnpm tauri bundle` sous Windows pour signer et empaqueter sans recompiler.
...
- Tauri CLI 2.10.0 restaure le binaire principal d'origine après chaque installateur : `release.yml` signe aussi ce binaire avant `tauri bundle` pour que la vérification finale `-ApplicationPath` reste valide ; le hook contrôle chaque copie modifiée avant emballage.
